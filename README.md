# Gautham YS

## Who am I?

A Master's student at the University of Illinois Chicago. 

## What I have built so far
1. Distributed Embedding and LLM Training in Apache Spark (UIC CS441 course project)
2. Embeddings for Music (UIC CS483 course project)
3. Transparency.ai - Transparent Hiring and ATS systems (UIC CS517 course project)
4. TeraOps - Quantum Compute Resource Optimization  (UIC CS532 course project)
5. gRPC v Thrift analysis - Physical, VMs and Docker

## What I want to build

See [Project Ideas](https://www.github.com/gauthamys/project-ideas)
(A repository of project ideas with me and [@sahanajprakash](https://www.github.com/sahanajprakash))

Feel free to hit me up or get in touch (gsaty@uic.edu) if any of these ideas interests you, we'd love to work together!

